...
def _kv(label: str, value: Any, color: str = "") -> str:
...
def _section_requests(d: dict[str, Any]) -> str:
    r = d.get("requests", {})
...
def _section_circuit_breakers(d: dict[str, Any]) -> str:
    breakers: list[dict[str, Any]] = d.get("circuit_breakers", [])
    if not breakers:
...
def _section_link_health(d: dict[str, Any]) -> str:
    links: list[dict[str, Any]] = d.get("link_health", [])
    if not links:
...
def _section_slo(d: dict[str, Any]) -> str:
    slo = d.get("slo", {})
    violators: list[str] = d.get("slo_violators", [])
    vcount = slo.get("total_violations", 0)
    vcolor = _C["red"] if vcount else _C["green"]
    body = (
        _kv("SLO target (TTFT)", f"{slo.get('slo_ms', 500):.0f} ms")
...
def _section_memory(d: dict[str, Any]) -> str:
    mem = d.get("memory", {})
...
def _section_admission(d: dict[str, Any]) -> str:
    adm = d.get("admission", {})
...
def _section_models(d: dict[str, Any]) -> str:
    models: list[dict[str, Any]] = d.get("models", [])
    if not models:
...
def generate_dashboard_data(
    ...
    model_registry_fn: Any,  # callable -> list[dict]
) -> dict[str, Any]:
    ...
    return {
        ...
        "link_health": link_monitor.get_stats(),
        "slo": slo_summary,
        "slo_violators": slo_tracker.violating_clients()[:5],
        "memory": memory_monitor.stats(),
        "admission": admission_controller.stats(),
